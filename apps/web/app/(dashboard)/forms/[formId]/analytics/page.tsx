...
export default function AnalyticsPage({ params }: { params: Promise<{ formId: string }> }) {
  ...
  const { data: fieldStats, isLoading: fieldLoading } = trpc.analytics.getFieldStats.useQuery({
    formId,
  });
          ...
          <>
            {[
              ...
              },
              {
                ...
                value: stats?.avgCompletionTime ? `${stats.avgCompletionTime}s` : "—",
                icon: Clock,
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        ...
        <Card>
          <CardContent className="p-5">
            ...
            {firstVisField && (
              <p className="text-xs text-gray-400 mb-3">{firstVisField.responseCount} responses</p>
            )}
            {fieldLoading ? (
              <div className="h-48 bg-gray-50 animate-pulse rounded" />
            ) : barData.length > 0 ? (
                ...
                <div key={f.fieldId} className="border rounded-lg p-3">
                  ...
                  {f.avgRating !== undefined && (
                    <p className="text-sm text-gray-600">
                      Avg rating: <strong>{f.avgRating}</strong>
                    </p>
                  )}
                  {f.avgLength !== undefined && (
                    <p className="text-sm text-gray-600">
                      Avg length: <strong>{f.avgLength} chars</strong>
                    </p>
                  )}
                  {f.trueCount !== undefined && (
                    <p className="text-sm text-gray-600">
                      Checked: <strong>{f.trueCount}</strong> · Unchecked:{" "}
                      <strong>{f.falseCount}</strong>
                    </p>
                  )}
                  {f.distribution && Object.keys(f.distribution).length > 0 && (
                    <div className="mt-2 space-y-1">
                      {Object.entries(f.distribution)
                        ...
                        .map(([k, v]) => {
                          const pct =
                            f.responseCount > 0 ? Math.round((v / f.responseCount) * 100) : 0;
                          return (
            ...
            <div className="divide-y">
              {responsesData.responses.map((r) => (
                <div key={r.id} className="flex items-center justify-between py-2 text-sm">
                  ...
                  <span className="text-gray-400 text-xs">
                    {r.completionTime ? `${r.completionTime}s` : "—"}
                  </span>
